...
public sealed class SimWorld
{
    ...
    internal EntityId Spawn(SpawnSpec spawn)
    {
        ...
        var id=Entities.Create();int i=id.Index;
        ...
        if(!spawn.Building){var d=DefDatabase.Units[spawn.Definition];Entities.Movement[i].Speed=d.Speed;Entities.Movement[i].Airborne=d.Airborne;Entities.Movement[i].Radius=Fix64.FromRatio(1,4);}
    ...
    {
        if((uint)c.Type>(uint)CommandType.Repair || !Entities.IsAlive(c.Entity) || Entities.Owner[c.Entity.Index].Player!=c.Player){_events.Enqueue(new(_clock.Tick,c.Player,"Rejected",c.Entity));return false;}
    ...
    {
        int i=c.Entity.Index;_active[i]=c;_states[i]=CommandStateMachine.Next(c.Type);
