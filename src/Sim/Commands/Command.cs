...
public enum CommandType { Move, AttackMove, Attack, Stop, Hold, Patrol, Follow, Build, Train, Cancel, Rally, Gather, Repair }
public readonly record struct Command(CommandType Type, int Player, EntityId Entity, EntityId Target, Fix2 Position, int Definition = -1, bool Queued = false)
{
    ...
    {
        if(bytes.Length!=ByteSize || bytes[0]>(byte)CommandType.Repair || bytes[1]>1 || bytes[2]!=0 || bytes[3]!=0 || BinaryPrimitives.ReadInt32LittleEndian(bytes.Slice(44))!=0)
