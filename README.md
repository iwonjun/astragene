...
### 설치 없이 브라우저로 참가
호스트가 **배포 zip의 AgeOfDominion.exe**(또는 `./tools/build_release.ps1` 후 소스 실행)로 방을 만들면, 같은 PC가
`http://호스트IP:27502` 로 웹 버전을 제공합니다. 친구는 그 주소를 브라우저로 열고 **[접속]** 만 누르면 됩니다(주소 자동 입력).

### 친구와 접속할 때
- 포트: 데스크톱 **27500/UDP**(ENet), 브라우저 게임 **27501/TCP**(WebSocket), 웹 페이지 **27502/TCP**(HTTP). Windows 방화벽 허용 창이 뜨면 허용하세요.
...
## 배포 빌드
`./tools/build_release.ps1` → `builds/AgeOfDominion-windows.zip`(Windows 실행 파일 + web/ 폴더), `builds/web/`.
표준 Godot 4.6.3 내보내기 템플릿(Windows, Web)이 `%APPDATA%/Godot/export_templates/4.6.3.stable`에 필요합니다.
