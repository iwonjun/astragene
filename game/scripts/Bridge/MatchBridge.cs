...
public partial class MatchBridge : Node
{
    ...
    internal void Initialize(SimWorld world) { World=world; _pending.Clear(); }
    ...
    public Godot.Collections.Array<int> FriendlyIds()
    {
        ...
        {var id=World.Entities.IdAt(i);if(id!=EntityId.None && World.Entities.Get(id).Owner.Player==LocalPlayer)result.Add(i);}
        return result;
    }
}
